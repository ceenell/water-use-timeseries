...
wu_keep <- wu_states %>% select(state_name, year, contains('Mgal') & contains('withdrawals') & contains('total'),
...
wu_clean <- wu_vars %>%
  left_join(wu_keep) %>%
  mutate(across(where(is.double) | where(is.character) & !c(state_name, year), ~as.numeric(.x))) %>%
  pivot_longer(!c(state_name, year), "use", "use_value") %>%
...
# filter to withdrawal total variables - includes overall total and by saline / fresh & broken down by gw/sw
wu_keep_county <- wu_county %>% select(state_name, county_cd, county_nm, year, contains('Mgal') & contains('withdrawals') & contains('total'),
                                -contains("Hydroelectric"), -contains("Fossil"), -contains("Geothermal"),-contains("Nuclear"), 
                                -contains("Closed"), -contains("Once"), -contains('Deliveries'))  # drop subcategories in thermoelectric and irrigation
wu_keep_county
...
wu_clean_county <- wu_vars_county %>%
  left_join(wu_keep_county) %>%
  mutate(across(where(is.double) | where(is.character) & !c(state_name, county_cd, county_nm, year), ~as.numeric(.x))) %>%
...
# create new variables based on type of water for surface water, groundwater, saline, and freshwater
wu_clean_time_county <- wu_clean_county %>% 
  filter(!is.na(water_use)) %>%
  mutate(water_kind = case_when(
    water_source == "total" & water_type == "both" ~ 'total',
    water_source == "total" & water_type == "fresh" ~ 'fresh',
  ...
  summarize(value = sum(value, na.rm = TRUE)) %>%
  dcast(state_name + year + water_use ~ water_kind, value.var = "value") %>%
  left_join(wu_vars_county)
wu_clean_time_county

# export county-level timeseries of water use categories by type/source
# # using multiple total columns to account for differences in reporting
wu_clean_time_county %>% 
  select(state_name, year, water_use, sw, gw, saline, fresh, total) %>%
  mutate(total_type = saline+fresh, total_source = gw+sw) %>%
  write_csv("/Users/cnell/Documents/Projects/water-use-timeseries/out/use_county_time.csv")

##
## generate timeseries data on population trend and public supply
ps_keep <- wu_county %>% select(contains("Public.Supply"), -contains("deliveries"))

ps_clean <- wu_county[,c(1:5)] %>%
  cbind(ps_keep) %>% 
  mutate(across(where(is.double) | where(is.character) & !c(state_cd, county_cd, county_nm, state_name, year), ~as.numeric(.x))) %>%
  pivot_longer(!c(state_cd, county_cd, county_nm, state_name, year), "use", "use_value") %>% 
  mutate(used = use, use = str_split(tolower(use), '\\.')) %>% 
  rowwise() %>%
  mutate(water_source = case_when(
    'groundwater' %in% unlist(use) ~ 'gw',
  ...
  water_use =  'public supply') %>%
  ungroup()%>%
  dcast(state_cd+state_name+county_nm+county_cd+year+water_use~used, value.var = "value")

colnames(ps_clean) <- c("state_cd", "state_name","county_nm", "county_cd", "year", "water_use", "ps_facilities", "ps_gallons_person_day", "ps_pop_gw_1000", "ps_pop_sw_1000",
                        "ps_use_loss", "ps_reclaimed", "ps_gw_fresh", "ps_gw_saline", "ps_sw_fresh", "ps_sw_saline", "ps_pop_served_1000", "ps_fresh", "ps_gw","ps_saline","ps_sw","ps_total")
